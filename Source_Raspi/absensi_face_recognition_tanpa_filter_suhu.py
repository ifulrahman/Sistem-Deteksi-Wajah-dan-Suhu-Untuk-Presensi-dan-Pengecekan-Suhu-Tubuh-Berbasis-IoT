...
url_absensi = Server+"/husni/api/absensi"
url_addFaceID = Server+"/husni/api/addfaceid"
url_delFaceID = Server+"/husni/api/delfaceid"
url_confirmFaceID = Server+"/husni/api/confirm"
url_name_user = Server+"/husni/api/listfaceid"
...
def absensi():          #absensi
    ...
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    recognizer.read('/home/pi/Absensi/Source_Raspi/absensi/include/trainer.yml')
    cascadePath = "/home/pi/Absensi/Source_Raspi/absensi/include/face.xml"
    faceCascade = cv2.CascadeClassifier(cascadePath);
    Id = 0
    face_id = 0
            ...
            for(x,y,w,h) in faces:
                ...
                if corX < 80 or corX > 560:
                    ...
                    if a < 8 and b < 8:
                        ...
                        suhuTubuh = (dataTemp[mapAddr[a][b]] + kalibrasi_suhu)
...
Copyright = Label(root, text="Capstone Design 2023",
              font=("arial", 10, "bold"), fg="red").place(x=170,y=600)
